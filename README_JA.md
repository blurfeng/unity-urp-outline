...
## サポートされるパラメータ
- **拡張アルゴリズム（Expand Mode）**：マスクからアウトラインを外側へどう広げるかを選択します。**JumpFlood**（既定）——真のユークリッド距離場で、どんな幅でも滑らかかつ均一な太さを保ちます；**SeparableBlur**——柔らかいグロー状のエッジ；**Dilate**——軽量な 8 サンプリング膨張。最も低コストですが、大きな幅では八角形に破綻します。  
- **HDR カラー**：アウトラインに使用する HDR カラー。発光効果も表現可能です。  
- **アウトライン幅**：UV サンプリングによって実装。調整範囲を拡張しました（上限 0.05）。JumpFlood / SeparableBlur では大きな幅でも滑らかさを保ち、Dilate のみ直角や四角形のエッジで八角形に破綻します。  
- **不透明度**：アウトライン全体の濃さ（0〜1）。  
- **エッジの硬さ**：エッジの減衰をべき乗カーブで整形します（大きいほど鋭く細く、小さいほど柔らかく太く）。低めの値にすると均一で塗りつぶされたアウトライン帯を再現できます。  
- **オクルージョンカリング（Occlusion Culling）**：有効にすると、他のジオメトリに隠れた部分はアウトライン描画されません（カメラ深度テクスチャは有効時のみ自動的に要求されます）。  
...
> 💡 透明 / アルファクリップ材質のオブジェクトも正しくアウトライン描画されるようになりました。  
> 💡 拡張アルゴリズム・カラー・幅・不透明度・エッジの硬さ・オクルージョンカリング・レンダリングレイヤーマスクはすべて **Volume** で実行時に上書きできます。Render Pass Event は **Renderer Feature** レベルの設定です。

### 拡張アルゴリズムの比較
下図は大きな幅での 3 つの拡張アルゴリズムの違いを比較したものです：Dilate は八角形に破綻し、JumpFlood は滑らかで均一な太さを保ち、SeparableBlur は柔らかいグローエッジになります。

![](Documents/ExpandMode.png)

### パフォーマンス比較
3 つの拡張アルゴリズムの相対コストは「フルスクリーンパス数 + ピクセルあたりのサンプリング数 + 中間 RT」から客観的に見積もれます（実際のコストは GPU・解像度・アウトライン幅によって変わるため、正確な値は Frame Debugger / GPU Profiler で計測してください）：

| 拡張アルゴリズム | フルスクリーンパス数 | 中間 RT | 幅に応じて増加 |
| --- | --- | --- | --- |
| **Dilate** | 1 | なし | いいえ |
| **SeparableBlur** | 3（固定） | 2× R8 | いいえ |
| **JumpFlood** | 2 + N（N ≈ ⌈log₂(幅 × 画面横方向ピクセル)⌉、目安 3~7） | 2× RGFloat | はい |

総合的なコストは **Dilate ＜ SeparableBlur ＜ JumpFlood** の順です：Dilate が最も軽量（解析パス 1 つのみ）；SeparableBlur は中程度で固定（横 + 縦ガウス + 解析の 3 パス、幅に依存しない）；JumpFlood は品質は最高ですがコストも最大——ジャンプ反復回数が幅に応じて log₂ で増え、さらに高精度 RGFloat の距離場を使うため帯域負荷も大きくなります。
