...
## 支持的参数
- **扩展算法（Expand Mode）**：决定如何从遮罩向外生成描边。**JumpFlood**（默认）——真实距离场，任意宽度都平滑等宽；**SeparableBlur**——辉光式软边；**Dilate**——轻量 8 抽样膨胀，开销最低，但大宽度会出现八边形穿帮。  
- **HDR 颜色**：描边使用的高动态范围颜色，可呈现发光效果。  
- **描边宽度**：基于 UV 采样实现，可调范围已放宽（上限 0.05）。在 JumpFlood / SeparableBlur 下大宽度仍保持平滑；仅 Dilate 会在直角或规则形状处出现八边形穿帮。  
- **不透明度**：描边整体浓淡（0–1）。  
- **边缘硬度**：边缘由柔和到锐利的幂次整形（越大越锐越细、越小越柔越粗）；较低取值可复现均匀实心的外描边带。  
- **遮挡剔除（Occlusion Culling）**：开启后，被其他物体遮挡的部分不再绘制描边（会自动请求相机深度图，仅在开启时请求）。  
- **渲染层遮罩**：通过渲染层遮罩控制哪些物体会被描边。  
- **注入时机（Render Pass Event）**：描边在渲染管线中的绘制阶段，属 Renderer Feature 级设置（不随 Volume 运行时切换），默认在后处理前绘制。  

> 💡 透明 / Alpha 裁剪材质的物体现在也能正确描边。  
> 💡 扩展算法、颜色、宽度、不透明度、边缘硬度、遮挡剔除、渲染层遮罩均可在 **Volume** 中运行时覆盖；注入时机为 **Renderer Feature** 级设置。

### 扩展算法对比
下图对比大宽度下三种扩展算法的差异：Dilate 出现八边形穿帮，JumpFlood 保持平滑等宽，SeparableBlur 呈辉光软边。
