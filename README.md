...
- **描边宽度**：基于 UV 采样实现，可调范围已放宽（上限 0.05）。若宽度过大，可能在直角或规则形状处出现穿帮现象。  
- **不透明度**：描边整体浓淡（0–1）。  
- **边缘硬度**：边缘由柔和抗锯齿到锐利可调，作为幂次整形（>1 更锐利更细，<1 更柔和更粗）。  
- **内部深度**：描边向物体内部渗入的深度，越大越深、越小越贴外缘。  
- **渲染层遮罩**：通过渲染层遮罩控制哪些物体会被描边。  
- **注入时机（Render Pass Event）**：描边在渲染管线中的绘制阶段，属 Renderer Feature 级设置（不随 Volume 运行时切换），默认在后处理前绘制。  

> 💡 颜色、宽度、不透明度、边缘硬度、内部深度、渲染层遮罩均可在 **Volume** 中运行时覆盖；注入时机为 **Renderer Feature** 级设置。
